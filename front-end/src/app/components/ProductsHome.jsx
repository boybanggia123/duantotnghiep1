...
import { addToFavourites, removeFromFavourites } from "../../redux/slices/favouriteSlice"; // Import action
...
function ProductsHome(props) {
  ...
  const toggleFavourite = (product) => {
    setFavouriteIds((prev) => {
      ...
      if (newFavourites.has(product._id)) {
        ...
        setNotification({ message: 'Đã xóa khỏi mục yêu thích!', type: 'error' }); // Cập nhật thông báo
      } else {
        ...
        setNotification({ message: 'Đã thêm vào mục yêu thích!', type: 'success' }); // Cập nhật thông báo
      }
  ...
  const notificationClass = notification.message ? `notification show ${notification.type}` : 'notification';
...
>>>>>>> 16db798c41636f66c45ff1b68ab0e0f2b3445528
                ...
                <div className="mt-2 fw-medium">
                  ...
                  <span className="price">
                    {discountedPrice > 0 ? (
                      <>
                        <p className="priceSale">${(price - (price * discountedPrice / 100))}</p>
                        <del>${price}</del>
                      </>
                    ) : (
              ...
              {discountedPrice > 0 && (
                <p className="discount position-absolute">{discountedPrice}% OFF</p>
              )}
